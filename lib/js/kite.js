...
var Deferred = provide.Deferred = Class.create(function(Deferred) {
  ...
  this.run = function run() {
    if (arguments.length) this._args = arguments;
    ...
    while (!this._paused && this._stack.length) {
      ...
      else if (res instanceof Deferred) this.join(res);
      else this._args = [res];
    }
  ...
  this.fn = function fn(callback, args, context) {
    ...
    var self = this;
    return function() {
      if (hasOkFn) self.pause();
      var ret = callback.apply(context, args);
      return hasOkFn ? self : ret;
    };
  };
